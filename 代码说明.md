...
将一个无缓冲chan转换为有缓冲chan

### subtract.go

对两个通道中的数值进行逐元素减法运算，返回包含计算结果的新通道。

### multiply.go

对两个通道中的数值进行逐元素乘法运算，返回包含计算结果的新通道。

### multiply_by.go 

将通道中的每个数值乘以指定倍数，返回包含计算结果的新通道。

###  divide.go

这段代码实现了两个通道中数值的逐元素除法运算。具体来说。

### divide_by.go 

对通道中每个元素进行固定值除法运算的功能。

###  change.go

计算数据流中每个元素与其N个位置前的元素之间的差值功能。

### change_ratio.go
计算数据流中每个元素与其N个位置前的元素之间的**比率变化（百分比变化）**功能。

### change_percent.go

计算百分比变化的功能，用于比较通道中当前值与指定位置前的值之间的百分比变化。
是change_ratio.go 的值 * 100

### count.go

生成一个数字序列，从指定的值开始，从1开始，递增1，直到给定的其他通道继续产生值。

### decrement_by.go

通道数据减法运算转换器，其核心作用是将输入通道中的每个数值元素减去指定值后输出到新通道。

### first.go

从通道（channel）中提取前N个元素的功能

### head.go

从通道（channel）中提取前N个数值类型元素的功能

### first.go

从通道（channel）中提取前N个元素的功能,并排空剩余chan

### increment_by.go

将输入通道中的每个元素按指定的增量值递增，并返回一个包含增量值的新通道。
